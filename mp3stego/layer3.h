 ...
 * was written. */
unsigned char *shine_encode_buffer(shine_t s, int16_t **data, int *written, int *frames_processed,
                                   long totalFrames);

/* Encode interleaved audio data. Source data must have `shine_samples_per_pass(s)` audio samples
 * per
 * channels. Mono encoder only expect one channel.
 *
 * Returns a pointer to freshly encoded data while `written` contains the size of
