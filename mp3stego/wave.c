...
unsigned char wave_open(const char *fname, wave_t *wave, shine_config_t *config, int quiet)
{
    ...
    wave->total_frame = (long)ceil(data_chunk.length / (2 * wave->channels) / (2 * 576));

    if (!quiet)
        printf("%s, %s %ldHz %ldbit, duration: %02ld:%02ld:%02ld\n", "WAVE PCM Data",
               channel_mappings[fmt_chunk.channels], (long)fmt_chunk.sample_rate,
               (long)fmt_chunk.depth, (long)wave->duration / 3600, (long)(wave->duration / 60) % 60,
