...
static void update_status(int frames_processed, long totalFrames)
{
    printf("\015[Frame %6d of %6ld] (%2.2f%%)", frames_processed, totalFrames,
           (float)((float)frames_processed / totalFrames) * 100);
    fflush(stdout);
}
...
int shine_samples_per_pass(shine_t s) { return s->mpeg.granules_per_frame * GRANULE_SIZE; }
...
static unsigned char *shine_encode_buffer_internal(shine_global_config *config, int *written,
                                                   int stride, int *frames_processed,
                                                   long totalFrames)
{

    config->mpeg.ext = getBit(*frames_processed);
    update_status(++(*frames_processed), totalFrames);
...
unsigned char *shine_encode_buffer(shine_global_config *config, int16_t **data, int *written,
                                   int *frames_processed, long totalFrames)
{
    config->buffer[0] = data[0];
    if (config->wave.channels == 2)
        config->buffer[1] = data[1];

    return shine_encode_buffer_internal(config, written, 1, frames_processed, totalFrames);
}

unsigned char *shine_encode_buffer_interleaved(shine_global_config *config, int16_t *data,
                                               int *written, int *frames_processed,
                                               long totalFrames)
{
    ...
    return shine_encode_buffer_internal(config, written, config->wave.channels, frames_processed,
                                        totalFrames);
}
